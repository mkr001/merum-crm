  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <div style={{ background: '#fff', borderRadius: 16, width: 540, maxHeight: '90vh', overflowY: 'auto', padding: 28 }}>
        <h2 style={{ margin: '0 0 20px', fontSize: 18, fontWeight: 700 }}>{form.id ? 'Edit Ticket' : 'New Support Ticket'}</h2>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          <div style={{ gridColumn: '1/-1' }}>
            <label style={labelStyle}>Subject *</label>
            <input style={inputStyle} value={form.subject || ''} onChange={e => set('subject', e.target.value)} placeholder="Brief summary of the issue" />
          </div>
          <div>
          ...
          <div style={{ gridColumn: '1/-1' }}>
            ...
            <textarea style={{ ...inputStyle, minHeight: 80, resize: 'vertical' }} value={form.description || ''} onChange={e => set('description', e.target.value)} placeholder="Detailed description…" />
          </div>
          {form.id && (
            <div style={{ gridColumn: '1/-1' }}>
              <label style={labelStyle}>Resolution Notes</label>
              <textarea style={{ ...inputStyle, minHeight: 60, resize: 'vertical' }} value={form.resolution_notes || ''} onChange={e => set('resolution_notes', e.target.value)} placeholder="How was this resolved?" />
            </div>
          )}
        </div>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: '#1a1a18' }}>Support Tickets</h1>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ background: '#f8f7f4', borderBottom: '1px solid #e8e6e0' }}>
              {['Subject', 'Client', 'Category', 'Priority', 'Status', 'Assigned To', 'Created', ''].map(h => (
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={8} style={{ textAlign: 'center', padding: 40, color: '#aaa' }}><MessageSquare size={32} style={{ margin: '0 auto 10px', display: 'block', opacity: 0.3 }} />No tickets found</td></tr>
            ) : tickets.map(t => {
              ...
              return (
                <tr key={t.id} style={{ borderBottom: '1px solid #f0ede8' }}>
                  ...
                  <td style={{ padding: '12px 14px', color: '#555' }}>{t.clients?.org_name || '—'}</td>
                  ...
                  <td style={{ padding: '12px 14px' }}>
                    <button onClick={() => setModal(t)} style={{ padding: '5px 12px', border: '1px solid #ddd', borderRadius: 6, background: '#fff', fontSize: 12, cursor: 'pointer' }}>Edit</button>
                  </td>
                </tr>
              );
            })}
      ...
      {modal && (
        <TicketModal
          ...
          onSave={() => { toast.success('Ticket saved!'); fetchAll(); }}
        />
      )}
    </div>
  );
